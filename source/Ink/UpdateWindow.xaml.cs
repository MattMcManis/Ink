    ...
    public partial class UpdateWindow : Window
    {
        ...
        public void StartDownload()
        {
            ...
            Thread worker = new Thread(() =>
            {
                ...
                Uri downloadUrl = new Uri("https://github.com/MattMcManis/Ink/releases/download/" + "v" + Convert.ToString(MainWindow.latestVersion) + "-" + MainWindow.latestBuildPhase + "/Ink.zip"); // v1.0.0.0-alpha/Ink.zip
                ...
                wc.DownloadFileAsync(downloadUrl, tempDir + "Ink.zip");

                // Progress Info
                progressInfo = "Downloading Ink...";
                ...
                List<string> extractArgs = new List<string>() {
                    ...
                    // Message
                    "Write-Host \"Updating Ink to version " + Convert.ToString(MainWindow.latestVersion) + ".\";",
                    "Write-Host \"`nPlease wait for program to close.\";",
                    // Wait
                    "timeout 3;",
                    // Extract
                    "$shell = new-object -com shell.application;",
                    "$zip = $shell.NameSpace('" + tempDir + "Ink.zip');",
                    "foreach ($item in $zip.items()) {$shell.Namespace('" + App.appDir + "').CopyHere($item, 0x14)};",
                    // Delete Temp
                    "Write-Host \"`nDeleting Temp File\";",
                    "del " + "\"" + tempDir + "Ink.zip" + "\";",
                    // Complete
                    "Write-Host \"`nUpdate Complete. Relaunching Ink.\";",
                    "timeout 3;",
                    // Relaunch Ink
                    "& '" + App.appDir + "Ink.exe'",
                };
